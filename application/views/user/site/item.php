		...
		<div class="item">
			<div class="box-item">
				<div class="image">
					...
					<a href="<?=ROOT?>san-pham/<?=$id?>-<?=url_encode($name)?>.html" title="<?=$name?>"><img src="<?=ROOT?>upload/product/450/<?=$image[0]?>" alt="<?=$name?>" ></a></div>
				<div class="info">
					<div class="price"><?=number_format($price)?> VNĐ</div>
					<a href="<?=ROOT?>san-pham/<?=$id?>-<?=url_encode($name)?>.html" title="<?=$name?>" class="name"><?=$name?></a>
				</div>
			</div>
		</div>
